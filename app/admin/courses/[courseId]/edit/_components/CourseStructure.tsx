...
import { DeleteLesson } from './DeleteLessonModal';
import { DeleteChapter } from './DeleteChapterModal';
...
export function CourseStructure({ data }: IAppProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === 'chapter') {
      ...
      //to be able to roll back to previous state if error occurs during api calls
    ...
    if (activeType === 'lesson' && overType === 'lesson') {
      ...
      if (chapterIndex === -1) {
        toast.error(
          'Could not find the chapter for the lesson being reordered',
        );
        return;
      }
      ...
      if (oldLessonIndex === -1 || newLessonIndex === -1) {
        toast.error('Could not find the lesson for reordering');
        return;
      }
