    ...
    if (decision.isDenied()) {
      if (decision.reason.isRateLimit()) {
        ...
        return {
          status: 'error',
          message:
            'Looks like you are a bot! If this is a mistake contact our support',
        };
      }
    }
...
export async function deleteChapter(
  ...
  try {
    ...
    const remaingingChapters = chapters.filter(
      (chapter) => chapter.id !== chapterId,
    );

    const updates = remaingingChapters.map((chapter, index) => {
